...
const validate = new Ajv({ allErrors: true }).compile(
  JSON.parse(readFileSync(resolve('tests/fixtures/reference/first-transition.schema.json'), 'utf8')),
);
const provenance = {
  ...
  combinedManifestSha256: manifestSha,
...
const input = {
  ...
  tokenRows: [
    ...
    [
      151644, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654,
      151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654,
      151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151654, 151645, 151669,
    ],
  ],
};
const parameters = {
...
const decision = {
  ...
  semanticCode: 8,
  residualCodes: [0, 1, 2, 3, 4, 5, 6],
};
const checkpoint = (name: string, dtype: string, shape: number[], value = 0) => ({
  name,
  dtype,
  shape,
  values: Array.from({ length: shape.reduce((size, dimension) => size * dimension, 1) }, () => value),
});
const receipt = (name: string, dtype: string, shape: number[], bytes: number) => ({
  name,
  path: `tensors/${name}.bin`,
  dtype,
  shape,
  bytes,
  sha256: sha,
...
const firstTransition = {
  schemaVersion: 2,
  scope: 'first-transition',
  provenance,
  input,
  parameters,
  decisions: [decision],
  checkpoints: [
    checkpoint('semantic-topk-ids', 'int32', [2, 50]),
    checkpoint('semantic-topk-logits', 'float32', [2, 50]),
    checkpoint('residual-topk-ids', 'int32', [7, 2, 50]),
    checkpoint('residual-topk-logits', 'float32', [7, 2, 50]),
    checkpoint('conditional-frame-hidden', 'bfloat16', [8, 4096]),
    checkpoint('feedback', 'bfloat16', [2, 1, 4096]),
...
const fullFiveSecond = {
  ...
  checkpoints: [],
  tensorReceipts: [
    receipt('frame-handoff', 'bfloat16', [1, 125, 32768], 8192000),
    receipt('condition', 'bfloat16', [1, 430, 2048], 1761280),
    receipt('initial-flow-noise', 'bfloat16', [1, 128, 430], 110080),
    receipt('flow-step-1', 'bfloat16', [1, 128, 430], 110080),
    receipt('flow-step-15', 'bfloat16', [1, 128, 430], 110080),
    receipt('flow-step-30', 'bfloat16', [1, 128, 430], 110080),
    receipt('final-latent', 'bfloat16', [1, 128, 430], 110080),
    receipt('waveform', 'float32', [1, 2, 220160], 1761280),
    receipt('wav', 'uint8', [880684], 880684),
  ],
...
describe('reference trace contract', () => {
  ...
  it('rejects invalid token and sampled-code ranges in both validators', () => {
    ...
    expectRejection({
      ...firstTransition,
      decisions: [{ ...decision, residualCodes: [...decision.residualCodes.slice(0, 6), 1_024] }],
    });
  });
  ...
  it('rejects the wrong fixed prompt, token rows, checkpoint shape, and receipt contract', () => {
    ...
    expectRejection({
      ...
      input: {
        ...input,
        tokenRows: input.tokenRows.map((row) => [...row]).map((row, index) => (index ? row : [...row, 1])),
      },
    });
    expectRejection({
      ...firstTransition,
      checkpoints: [{ ...firstTransition.checkpoints[0], shape: [1, 100] }, ...firstTransition.checkpoints.slice(1)],
    });
    expectRejection({
      ...fullFiveSecond,
      tensorReceipts: [
        { ...fullFiveSecond.tensorReceipts[0], dtype: 'float32' },
        ...fullFiveSecond.tensorReceipts.slice(1),
      ],
    });
  });
  ...
  it('requires exactly one first transition with seven residual codes', () => {
    ...
    expectRejection({
      ...firstTransition,
      decisions: [{ ...decision, residualCodes: decision.residualCodes.slice(0, 6) }],
    });
  });
  ...
  it('requires finite structured checkpoints and every full tensor receipt', () => {
    ...
    expectRejection({
      ...fullFiveSecond,
      tensorReceipts: fullFiveSecond.tensorReceipts.slice(0, -1),
    });
  });
  ...
  it('rejects duplicate, extra, and scope-inappropriate boundaries in both validators', () => {
    ...
    expectRejection({
      ...
      checkpoints: [
        firstTransition.checkpoints[0],
        { ...firstTransition.checkpoints[1], name: 'extra' },
        ...firstTransition.checkpoints.slice(2),
      ],
    });
    expectRejection({
      ...firstTransition,
      checkpoints: [{ ...firstTransition.checkpoints[0], extra: true }, ...firstTransition.checkpoints.slice(1)],
    });
    expectRejection({
      ...firstTransition,
      checkpoints: [
        ...firstTransition.checkpoints.slice(0, -1),
        { ...firstTransition.checkpoints.at(-1), values: [40, 42] },
      ],
    });
    expectRejection({
      ...firstTransition,
    ...
    expectRejection({
      ...fullFiveSecond,
      tensorReceipts: [
        fullFiveSecond.tensorReceipts[0],
        fullFiveSecond.tensorReceipts[0],
        ...fullFiveSecond.tensorReceipts.slice(2),
      ],
    });
    expectRejection({
      ...fullFiveSecond,
      tensorReceipts: [
        fullFiveSecond.tensorReceipts[0],
        { ...fullFiveSecond.tensorReceipts[1], name: 'extra' },
        ...fullFiveSecond.tensorReceipts.slice(2),
      ],
    });
    expectRejection({
      ...fullFiveSecond,
      tensorReceipts: [{ ...fullFiveSecond.tensorReceipts[0], extra: true }, ...fullFiveSecond.tensorReceipts.slice(1)],
    });
  });
  ...
  it('keeps forced sampled codes distinct from Gaussian flow noise', () => {
    expectRejection({
      ...firstTransition,
      decisions: [{ ...decision, kind: 'gaussian-flow-noise' }],
    });
    expectRejection({
      ...fullFiveSecond,
      flowNoise: { kind: 'uniform-draws', receiptName: 'initial-flow-noise' },
    });
  });
});
