...
describe('OpfsFp16EmbeddingTable', () => {
  it('looks up duplicated ids across a shard boundary in input order', async () => {
    const files = new Map([
      ...
      ['b', bytes([8, 9, 10, 11, 12, 13, 14, 15])],
    ]);
    const table = await OpfsFp16EmbeddingTable.open(
      {
        rows: 4,
        columns: 4,
        rowBytes: 8,
      ...
      async (path) => ({
        read(buffer, options) {
          ...
          buffer.set(source.slice(options.at, options.at + buffer.length));
  ...
  it('rejects invalid IDs and short reads, then closes opened handles', async () => {
    let closed = 0;
    const make = async (): Promise<SyncFileHandle> => ({
      read: () => 0,
      close: () => {
        closed++;
      },
    ...
    expect(() => table.lookup([1])).toThrow('out of range');
    expect(() => table.lookup([0])).toThrow('short read');
    table.dispose();
  ...
  it('closes an already-open shard when a later shard cannot open', async () => {
    ...
    await expect(
      OpfsFp16EmbeddingTable.open(table, async () => {
        if (calls++ === 1) throw new Error('second shard failed');
        return {
          read: () => 8,
          close: () => {
            closed++;
