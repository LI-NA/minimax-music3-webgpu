...
describe('qualification capture and Chrome profile paths', () => {
  it('requires a fresh nonempty child capture', async () => {
    ...
    const expected = path.join(checkout, 'artifacts', 'diagnostics', 'variable-duration', 'run-01');
    expect(paths?.resolveQualificationCapture(checkout, expected)).toBe(expected);
    expect(() =>
      paths?.resolveQualificationCapture(
        checkout,
        path.join(checkout, 'artifacts', 'diagnostics', 'variable-duration'),
      ),
    ).toThrow('nonempty child');
    expect(() => paths?.resolveQualificationCapture(checkout, path.resolve('outside'))).toThrow('contained');
  ...
  it('rejects a capture below an existing link that escapes its allowed root', async ({ skip }) => {
    ...
    const captureRoot = path.join(checkout, 'artifacts', 'diagnostics', 'variable-duration');
    ...
    try {
      ...
      expect(() => paths?.resolveQualificationCapture(checkout, path.join(escape, 'run-01'))).toThrow('contained');
    } finally {
  ...
  it('uses browser-profiles only in the primary checkout', async () => {
    ...
    expect(paths).toBeDefined();
    const checkout = path.resolve('synthetic-checkout');
    expect(paths?.resolveQualificationProfile(checkout, undefined, false)).toBe(
    ...
    expect(() =>
      paths?.resolveQualificationProfile(
        checkout,
  ...
  it('requires an explicit worktree-namespaced profile in linked worktrees', async () => {
    ...
    expect(paths).toBeDefined();
    const checkout = path.resolve('synthetic-checkout');
    expect(() => paths?.resolveQualificationProfile(checkout, undefined, true)).toThrow('explicit');
    const explicit = path.join(checkout, 'artifacts', 'worktree-profiles', 'feature-name', 'long-duration');
    ...
    expect(() =>
      paths?.resolveQualificationProfile(checkout, path.join(checkout, 'artifacts', 'worktree-profiles'), true),
    ).toThrow('nonempty child');
  });
});
