...
onnx.save_model(
    model,
    root / "external-add.onnx",
    save_as_external_data=True,
    all_tensors_to_one_file=True,
    location="external-add.bin",
    size_threshold=0,
)
...
matmul = helper.make_model(
    helper.make_graph(
        ...
        "q4-known-answer",
        [helper.make_tensor_value_info("input", TensorProto.FLOAT16, [1, 128])],
        [helper.make_tensor_value_info("output", TensorProto.FLOAT16, [1, 2])],
        [numpy_helper.from_array(packed, "weight"), numpy_helper.from_array(scales, "scales")],
    ),
