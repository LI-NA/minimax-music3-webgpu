...
export async function createWebGpuDevice(gpu: GPU, requiredLimits: Record<string, number> = {}): Promise<{ adapter: GPUAdapter; device: GPUDevice }> {
  ...
  const required = Object.fromEntries(Object.entries(requiredLimits).filter(([name, value]) => typeof limits[name] === 'number' && limits[name] >= value));
  if (Object.keys(required).length !== Object.keys(requiredLimits).length) throw new Error('Adapter limits are insufficient');
  return { adapter: capability.adapter, device: await capability.adapter.requestDevice({ requiredFeatures, requiredLimits: required }) };
}
